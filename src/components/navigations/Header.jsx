...
import { IconMoon, IconSun, IconMessageCircle } from '@tabler/icons-react'; // Import chat icon
...
function Header({ theme, toggleTheme }) {
  ...
  return (
    <div className={`fixed w-full px-4 top-0 py-4 z-1000 ${isDarkMode ? 'bg-black text-white' : 'bg-gradient-to-br from-[#cdfcff] via-[#a5d0e7] via-[#bcd9e9] via-[#fff] to-[#95cbe7] text-black'}`}>
      <div className="flex flex-row flex-wrap items-center justify-between lg:justify-between space-y-4 lg:space-y-0">
        ...
        {/* Notification, Chat Button, and User Dropdown */}
        <div className="flex-shrink-0 w-full lg:w-auto flex justify-center lg:justify-end space-x-4 items-center">
          <button onClick={toggleTheme} className={`p-3 rounded-full shadow-lg transition-all duration-300 ${
            ...
            {isDarkMode ? <IconMoon size={30} /> : <IconSun className="text-black" size={30} />}
          </button>
